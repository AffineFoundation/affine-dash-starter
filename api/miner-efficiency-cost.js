  ...
  try {
    const sql = `
      ...
      SELECT
          ...
          -- Use OUTPUT token price (per million tokens, USD) only.
          AVG((extra -> 'miner_chute' -> 'current_estimated_price' -> 'per_million_tokens' -> 'output' ->> 'usd')::double precision)::double precision as avg_token_cost_usd
      FROM
      ...
      HAVING
          ...
          AND AVG((extra -> 'miner_chute' -> 'current_estimated_price' -> 'per_million_tokens' -> 'output' ->> 'usd')::double precision) > 0
      ORDER BY
          hotkey;
    `;
