...
type EnvironmentsProviderProps = React.PropsWithChildren<{
  initialEnvironments?: string[];
}>;

export const EnvironmentsProvider: React.FC<EnvironmentsProviderProps> = ({
  children,
  initialEnvironments,
}) => {
  const [environments, setEnvironments] = useState<string[]>(() =>
    Array.isArray(initialEnvironments) ? initialEnvironments : [],
  );
  const [loading, setLoading] = useState<boolean>(
    !initialEnvironments || initialEnvironments.length === 0,
  );
