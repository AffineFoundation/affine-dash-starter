 ...
 */
export default async function networkActivityHandler(req, res) {
  if (req.method !== 'GET') {
  ...
  try {
    ...
    console.log(`Network activity query returned ${rows.length} rows`);
    ...
    console.error('Network activity query error:', {
      error: err.message,
    ...
    return res.status(500).json({
      ...
      message: 'Failed to fetch network activity data',
      timestamp: new Date().toISOString()
    });
  }
}