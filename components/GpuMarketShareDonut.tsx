...
import { GpuMarketShareRow } from '@/lib/types'
import {
  ResponsiveContainer,
  PieChart,
  Pie,
...
function buildSlices(rows: GpuMarketShareRow[]): Slice[] {
  ...
  const all: Slice[] = Array.from(counts.entries()).map(([name, value]) => ({
    name,
    value,
  }))
...
const GpuMarketShareDonut: React.FC<Props> = ({ theme, data, errorMessage }) => {
