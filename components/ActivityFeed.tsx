...
import { fetchActivity } from '@/lib/services/api'
...
const ActivityFeed: React.FC<ActivityFeedProps> = ({ theme, initialData, errorMessage, limit = 10 }) => {
  const [rows, setRows] = useState<ActivityRow[]>(() =>
    Array.isArray(initialData) ? initialData.slice(0, limit) : [],
  )
  const [manualRefreshing, setManualRefreshing] = useState(false)
  const [error, setError] = useState<string | null>(errorMessage ?? null)

  useEffect(() => {
    setRows(Array.isArray(initialData) ? initialData.slice(0, limit) : [])
  }, [initialData, limit])
  ...
  const handleRefresh = async () => {
    ...
    try {
      const latest = await fetchActivity({ cache: 'no-store' })
      if (Array.isArray(latest)) {
        setRows(latest.slice(0, limit))
      }
      setError(null)
    } catch (err) {
  ...
  const isLoading = rows.length === 0 && !error
