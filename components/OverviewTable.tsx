...
import { enrichLiveSubnetRows } from '@/lib/services/api'
import type { SubnetOverviewRow, LiveEnrichmentRow } from '@/lib/types'
...
const OverviewTable: React.FC<OverviewTableProps> = ({ theme, historicalData, historicalError }) => {
  ...
  const historicalRows: HistoricalDisplayRow[] = useMemo(() => {
  ...
  const historicalQueryError = historicalError ?? null
  const isHistoricalLoading = historicalRows.length === 0 && !historicalQueryError
