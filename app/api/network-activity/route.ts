...
import type { NetworkActivityRow } from '@/lib/types';

export const runtime = 'nodejs';

const NETWORK_ACTIVITY_SQL = `
  WITH windowed AS (
...
export async function GET() {
  try {
    const { rows } = await query<NetworkActivityRow>(NETWORK_ACTIVITY_SQL);
    return NextResponse.json(rows);
  } catch (error) {
    console.error('network-activity query error:', error);
